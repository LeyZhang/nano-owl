...
idx = 0
def detect(image, pos_prompt, neg_prompt, threshold, nms_threshold, conf_ratio, inter_ratio_threshold, min_box_size, ref_prompt_list):
    global fewshot_predictor 

    text = ref_prompt_list
...
def detect_vision_api(image_path, neg_prompt, threshold, nms_threshold, conf_ratio, inter_ratio_threshold, min_box_size, prompt_ref:list):
    input_image = Image.open(image_path)
    # ref_images = [Image.open(image_ref_path) for image_ref_path in image_ref_paths]

    result, result_filtered, log, embeds = detect(input_image, None, None, threshold, nms_threshold, conf_ratio, inter_ratio_threshold, min_box_size, prompt_ref)
...
if __name__ == "__main__":
    ...
    for image_name in tqdm(os.listdir(input_folder)):
        ...
        # 执行检测并打印结果
        try:
            labels, scores, boxes, indices, selected_embeds = detect_vision_api(input_image_path, neg_prompt, threshold, nms_threshold, conf_ratio, inter_ratio_threshold, min_box_size, prompt)
